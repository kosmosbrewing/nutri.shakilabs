...
- 상태: Implemented v1.0
- 기준일: 2026-07-14
- 산식 버전: `value-v1`, `unit-price-v1`, `category-value-v1`
...
- 비타민D, 비타민C, 칼슘, 프로바이오틱스, 오메가3, 마그네슘, MSM, 코엔자임Q10, 밀크씨슬은 각각 공식 핵심 함량과 가격 근거를 갖춘 `unit-price-v1` 비교를 제공한다.
- 9개 카테고리는 각 3개 검증 오퍼를 가지며 카테고리 사이의 가격효율 점수는 비교하지 않는다.
...
| 칼슘 | 1일 칼슘 | 100 mg당 가격 |
| 프로바이오틱스 | 1일 보장균수 | 10억 CFU당 가격 |
| 오메가3 | 1일 EPA+DHA | 100 mg당 가격 |
| 마그네슘 | 1일 마그네슘 | 100 mg당 가격 |
| MSM | 1일 MSM | 1,000 mg당 가격 |
| 코엔자임Q10 | 1일 코엔자임Q10 | 100 mg당 가격 |
| 밀크씨슬 | 1일 실리마린 | 100 mg당 가격 |
...
```text
categoryValue = (sameCategoryMinimumUnitPrice / productUnitPrice) * 100
```

- 같은 카테고리의 현재 검증 오퍼만 분모 집합에 포함한다.
- 최저 단위가격 제품은 100, 나머지는 100 이하로 표시한다.
- 가격이 바뀌면 같은 카테고리 전체를 다시 계산한다.
- 이 값은 절대 품질, 흡수율, 개인 적합성 또는 복용 권장 점수가 아니다.
